...
/**
 * Wrap each test body so window.gtag is always installed before and removed
 * after. Replaces beforeEach/afterEach hooks (not yet exported by node:test
 * in older @types/node versions).
 */
function withFakeGtag(fn: () => void) {
  installFakeGtag()
  try {
    fn()
  } finally {
...
test('all helpers: silently no-op when window is undefined (SSR path)', () => {
  removeWindow()
  calls.length = 0
